...
function LeaveCalendar () {
  ...
  const handleDateChange = (evt, name) => {
    const value = evt.target.value;
  ...
  const getEmployeeLeaves = async () => {
    ...
    const result = await employeeRequests.getEmployeesLeavesApi(
    ...
    result.data.map(x => {
      ...
      tempArr.push({
        ...
        daysLeaves: x.LeavesDates.map(leave => {
          return moment(leave).format('MMM Do YY') + ' , '
        }),
        action:
  ...
  const onChangeDate = date => {
    if (editLeavesClicked) {
      ...
      if (isAlreadyMarked) {
        ...
        setLeavesData({
          ...leaveData,
          LeavesDates: [...leaveData.LeavesDates, date]
        })
      ...
      <>
        ...
        {!addClick && !editLeavesClicked ? (
          ...
          <>
            ...
            <div className="form-group col-sm-4 flex-column d-flex" style={{ marginTop: "12px" }}>
              <label className="form-control-label">
                From
              </label>
              <input
                style={{ marginTop: "0px" }}
                onChange={(evt) => handleDateChange(evt, 'from')}
                type="date"
            ...
            <div className="form-group col-sm-4 flex-column d-flex" style={{ marginTop: "12px" }}>
              ...
              <input
                onChange={(evt) => handleDateChange(evt, 'to')}
                type="date"
                style={{ marginTop: "0px" }}
            ...
            <div className="form-group col-sm-4 flex-column d-flex" style={{ marginTop: "32px" }}>
         ...
         {columnsAndRows&&columnsAndRows?.rows?.length>0?
            <MDBDataTable
          ...
          <div className='calendar-container'>
            <div className='row justify-content-between text-left'>
              <div className='form-group col-sm-6 flex-column d-flex'>
                <label className='form-control-label px-3'>Leave Type </label>{' '}
                <Select
                  options={[
              ...
              <div className='form-group col-sm-6 flex-column d-flex'>
                <label className='form-control-label px-3'>Reason </label>{' '}
                <input
            ...
            <Calendar
              onChange={date => onChangeDate(moment(date).format('MM/DD/YYYY'))}
              tileDisabled={({ date, view }) =>
                (view === 'month' && date.getDay() === 0) ||
                date.getDay() === 6 ||
                mark.find(x => x === moment(date).format('MM/DD/YYYY'))
              }
              tileClassName={({ date, view }) => {
                if (
                  leaveData.LeavesDates.find(
                    x => x === moment(date).format('MM/DD/YYYY')
                  )
                ) {
                  return 'highlight'
                }
          ...
          <div className='calendar-container'>
            ...
            <Calendar
              ...
              tileDisabled={({ date, view }) =>
                ...
                mark.find(
                  x =>
                    !leaveData.LeavesDates.includes(x) &&
                    x === moment(date).format('MM/DD/YYYY')
                )
              }
              tileClassName={({ date, view }) => {
                if (
              ...
              activeStartDate={
                new Date(
                  leaveData.LeavesDates &&
                    leaveData.LeavesDates.length > 0 &&
                    leaveData.LeavesDates[0]
                )
              }
            />{' '}
            <div className='row justify-content-between text-left'>
